...
interface AnalysisGraphBuilderEvaluationArtifact {
  createdAt?: string;
  runId?: string;
  method?: string;
  parameters?: {
...
type TaxonomyPlotLayout = "taxonomy" | "vector";

interface TaxonomyPlot {
  layout: TaxonomyPlotLayout;
...
async function loadAnalysisRun(
  ...
  const graphBuilderEvaluation = await readJsonFile<AnalysisGraphBuilderEvaluationArtifact>(
    path.join(runDir, "graph-eval", "summary.json")
  );
...
async function loadTaxonomyPlot(
  ...
  if (layout === "taxonomy") {
    return buildTaxonomyLayoutPlot(taxonomyRun);
  }

  return loadTaxonomyVectorOverlayPlot(projectPaths, taxonomyRun);
}

async function loadTaxonomyVectorOverlayPlot(
  projectPaths: ProjectPaths,
  taxonomyRun: LoadedTaxonomyRun
): Promise<TaxonomyPlot> {
  const analysisRunId =
  ...
  if (analysisRunId === null) {
    return { layout: "vector", analysisRunId: null, clusterArtifactPath: null, points: [] };
  }
  ...
  if (best === null || best.matchCount === 0) {
    return { layout: "vector", analysisRunId, clusterArtifactPath: null, points: [] };
  }
...
function buildTaxonomyLayoutPlot(taxonomyRun: LoadedTaxonomyRun): TaxonomyPlot {
  ...
  type TaxonomyTheme = NonNullable<TaxonomyArtifact["themes"]>[number];
  const themesByCategory = new Map<string, TaxonomyTheme[]>();

  for (const category of categories) {
    themesByCategory.set(category.category_id as string, []);
  }

  for (const theme of themes) {
    const categoryId = theme.parent_category_id;

    if (typeof categoryId !== "string") {
      continue;
    }

    const categoryThemes = themesByCategory.get(categoryId) ?? [];
    categoryThemes.push(theme);
    themesByCategory.set(categoryId, categoryThemes);
  }

  const categoryCount = Math.max(categories.length, 1);
  const columns = categoryCount <= 2 ? categoryCount : categoryCount <= 4 ? 2 : 3;
  const rows = Math.ceil(categoryCount / columns);
  const cellWidth = 1 / columns;
  const cellHeight = 1 / rows;
  const pointRadius = Math.min(cellWidth, cellHeight);

  return {
    layout: "taxonomy",
    analysisRunId: null,
    clusterArtifactPath: null,
    points: taxonomyRun.assignments.flatMap((assignment) => {
      if (typeof assignment.opinion_id !== "string") {
        return [];
      }

      const categoryId = assignment.primary_category_id ?? null;
      const categoryPosition =
        categoryId === null ? -1 : (categoryIndex.get(categoryId) ?? categories.length);
      ...
      const column = categoryPosition % columns;
      const row = Math.floor(categoryPosition / columns);
      const categoryCenterX = column * cellWidth + cellWidth / 2;
      const categoryCenterY = row * cellHeight + cellHeight / 2;
      const categoryThemes = categoryId === null ? [] : (themesByCategory.get(categoryId) ?? []);
      const themePosition = categoryThemes.findIndex(
        (theme) => theme.theme_id === assignment.primary_theme_id
      );
      const themeCount = Math.max(categoryThemes.length, 1);
      const subgroupRadius = pointRadius * (themeCount <= 1 ? 0 : themeCount <= 3 ? 0.16 : 0.22);
      const angle =
        themePosition >= 0
          ? -Math.PI / 2 + (Math.PI * 2 * themePosition) / themeCount
          : deterministicAngle(`${assignment.opinion_id}:theme`);
      const subgroupCenterX = categoryCenterX + Math.cos(angle) * subgroupRadius;
      const subgroupCenterY = categoryCenterY + Math.sin(angle) * subgroupRadius;
      const jitterRadius = pointRadius * 0.055;
      const x =
        subgroupCenterX +
        (deterministicUnit(`${assignment.opinion_id}:x`) - 0.5) * jitterRadius * 2;
      const y =
        subgroupCenterY +
        (deterministicUnit(`${assignment.opinion_id}:y`) - 0.5) * jitterRadius * 2;
...
function renderTaxonomyRunPage(
  ...
  return renderPage(
    ...
    `<main class="shell">
      ...
      <section class="panel" id="taxonomy-map">
        <article class="card">
          <div class="section-head">
            <div>
          ...
          <div class="taxonomy-toolbar">
            <a class="button-link button-link-secondary button-link-small ${selectedCategory === null ? "active" : ""}" href="${buildTaxonomyHref(null, plot.layout, "#taxonomy-map")}">All</a>
            ${categories
              .map(
                (category) =>
        ...
        <section class="taxonomy-category-grid">
          ${categories
            .map((category, index) => {
              ...
              }#taxonomy-subgroups" style="--taxonomy-color:${escapeHtmlAttribute(color)}">
      ...
      <section class="panel" id="taxonomy-subgroups">
        <div class="section-head">
          <div>
...
function renderAnalysisDiagnosticsSummarySection(
  ...
  const graph = run.graphBuilderEvaluation;
  const hasDiagnostics = reducer !== null || clustering !== null || graph !== null;
      ...
      ${
        hasDiagnostics
          ? `<div class="meta-chip-row">
              ...
              ${renderMetaChip(`Graph ${escapeHtml(graph?.method ?? "missing")}`)}
              ${renderMetaChip(`${escapeHtml(String(reducer?.corpus?.comparableOpinionCount ?? clustering?.corpus?.comparableOpinionCount ?? graph?.corpus?.comparableOpinionCount ?? 0))} comparable opinions`)}
            </div>
            ${renderDiagnosticObservationPreview(reducer, clustering, graph)}`
...
node packages/cli/dist/index.js analysis --evaluate-clustering-surfaces --run ${escapeHtml(runId)}
node packages/cli/dist/index.js analysis --evaluate-graph-builders --run ${escapeHtml(runId)}</pre>`
      }
    </article>
  </section>`;
}
...
function renderDiagnosticObservationPreview(
  ...
  graph: AnalysisGraphBuilderEvaluationArtifact | null = null
): string {
  const observations = [
...
function renderGraphBuilderDiagnosticOverviewCard(
  graph: AnalysisGraphBuilderEvaluationArtifact | null
): string {
  if (graph === null) {
    return `<article class="card">
      <p class="eyebrow">Graph Builders</p>
      <h2>Not generated</h2>
      <p class="meta">Run <code>broadly analysis --evaluate-graph-builders</code> for this analysis run.</p>
    </article>`;
  }

  const bestGraphSurface = [...(graph.surfaces ?? [])]
    .filter(
      (item) =>
        item.surfaceKind === "graph" &&
        item.embeddingNeighborPurityAtK !== null &&
        item.embeddingNeighborPurityAtK !== undefined
    )
    .sort(
      (left, right) =>
        (right.embeddingNeighborPurityAtK ?? -1) - (left.embeddingNeighborPurityAtK ?? -1)
  ...
  return `<article class="card">
    <p class="eyebrow">Graph Builders</p>
    <h2>${escapeHtml(bestGraphSurface?.label ?? "No ready graph surfaces")}</h2>
    ${renderKeyValueList([
...
function renderGraphBuilderDiagnosticSection(
  graph: AnalysisGraphBuilderEvaluationArtifact | null
): string {
  if (graph === null) {
    return "";
  ...
  return `<section class="panel">
    <div class="section-head">
      <div>
        <p class="eyebrow">Graph Builders</p>
        <h2>Neighborhood graph variants</h2>
      </div>
    </div>
    <section class="grid">
      ${(graph.graphs ?? [])
        .map(
          (item) => `<article class="card">
            <p class="eyebrow">${escapeHtml(item.method ?? "graph")}</p>
            <h3>${escapeHtml(item.label ?? item.graphId ?? "unknown")}</h3>
            ${renderKeyValueList([
              ["Nodes", String(item.nodeCount ?? 0)],
...
function renderGraphComparisonSection(
  graph: AnalysisGraphBuilderEvaluationArtifact | null
): string {
  if (graph === null) {
    return "";
  ...
  const comparisons = (graph.comparisons ?? []).filter(
    (comparison) =>
      graphSurfaceIds.has(comparison.leftSurfaceId ?? "") ||
      graphSurfaceIds.has(comparison.rightSurfaceId ?? "")
  );

  return `<section class="panel">
    <div class="section-head">
      <div>
        <p class="eyebrow">Graph Agreement</p>
        <h2>Graph memberships vs baselines</h2>
      </div>
    </div>
    ${
      comparisons.length === 0
        ? `<article class="card"><p>No graph comparisons were available.</p></article>`
        : `<section class="record-list">
            ${comparisons
              .slice(0, 12)
              .map(
...
type DiagnosticMembershipShiftList = Array<{
  sourceClusterId?: number;
  sourceClusterSize?: number;
  fragmentationRate?: number;
  topDestinationClusters?: Array<{
...
function renderMembershipShiftList(
  shifts: DiagnosticMembershipShiftList | undefined
): string {
