...
| `broadly analysis --promote-graph-surface [surfaceId]` | Promote a graph diagnostic surface into a semantic-neighborhood analysis view. |
...
diagnostic surface into the normal analysis artifact set as a **Semantic
Neighborhood View**. If no `surfaceId` is provided, the command selects the
strongest repaired mutual-kNN graph surface by embedding-neighbor purity.
Promotion writes:
...
prompts. Promoted neighborhood views are marked experimental and are included in
report bundles as additional views; `report.primaryView` remains unchanged.

If a graph cluster contains at least 40% of the assigned opinions, it is labeled
`Mixed General Remainder` and held out of semantic merge themes and perspective
highlights. The cluster remains present in the cluster artifact and web
exploration views so it can be inspected or reclustered, but the report should
not treat it as one coherent theme.
...
- semantic-neighborhood analysis views promoted from graph diagnostics
